...
const schema = z.object({
  ...
  game_starts_at: z.string().optional(),
...
export const ReportBetDialog = ({ trigger }: { trigger?: React.ReactNode }) => {
  ...
  const { games: liveGames, loading: loadingGames } = useRealOdds("soccer_epl");
  ...
  const selectedGame = gameOptions.find((game) => game.id === form.game_id);

  const pickOptions = useMemo(() => {
    if (!selectedGame) return [];
    return [
      { value: selectedGame.homeTeam, label: selectedGame.homeTeam },
      { value: selectedGame.awayTeam, label: selectedGame.awayTeam },
      { value: "Draw", label: txt("תיקו", "Draw") },
    ];
  }, [selectedGame, lang]);
  ...
  const handleGameSelect = (gameId: string) => {
    const game = gameOptions.find((entry) => entry.id === gameId);
    if (!game) return;

    setForm((current) => ({
      ...current,
      game_id: game.id,
      game_starts_at: game.startTime,
      sport: game.sport,
      league: game.league,
      home_team: game.homeTeam,
      away_team: game.awayTeam,
      bet_on:
        current.bet_on === current.home_team || current.bet_on === current.away_team || current.bet_on === "Draw"
          ? ""
          : current.bet_on,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured) {
      toast.error(txt("דיווח הימורים עדיין לא מחובר בפרסום הזה", "Bet reporting is not connected in this deployment yet"));
      return;
    }
    ...
    const { error } = await supabase.from("bets").insert([{
      ...
      game_starts_at: d.game_starts_at || null,
    ...
    <Dialog open={open} onOpenChange={handleOpen}>
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 mt-2">
          <div>
            <Label>{txt("בחר משחק חי", "Choose a live game")}</Label>
            <Select value={form.game_id} onValueChange={handleGameSelect}>
              <SelectTrigger>
                <SelectValue
                  placeholder={
                    loadingGames
                      ? txt("טוען משחקים...", "Loading games...")
                      : txt("בחר משחק כדי למלא אוטומטית", "Pick a game to autofill")
                  }
                />
              </SelectTrigger>
              <SelectContent>
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              {selectedGame
                ? txt(
                    `${selectedGame.league} · ${new Date(selectedGame.startTime).toLocaleString("he-IL")}`,
                    `${selectedGame.league} · ${new Date(selectedGame.startTime).toLocaleString()}`
                  )
                : txt("אפשר גם להמשיך ידנית אם המשחק לא מופיע", "You can still continue manually if the game is missing")}
            </p>
          </div>
          ...
          <div>
            ...
            {pickOptions.length > 0 ? (
              <Select value={form.bet_on} onValueChange={(v) => setForm({ ...form, bet_on: v })}>
                <SelectTrigger>
                  <SelectValue placeholder={txt("בחר צד", "Choose a side")} />
                </SelectTrigger>
                <SelectContent>
